...
class TimelinesController extends \BaseController {
	...
	public function process(){
		...
		$campaigns = DB::table('campaigns')->where('is_published', '=', true)->get();

		//loop through campaigns and fetch timelines due
		foreach ($campaigns as $campaign) {
			$time = Carbon::now()->toStringFormat();
			$timelines_due_id = DB::table('timelines')->where('campaign_id', '=', $campaign->id)->where('time', '=', $time)->pluck('id');
			
			$timelines_due[] = $timelines_due_id;
		}	
	}
